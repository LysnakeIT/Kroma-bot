...
module.exports = {
    ...
    run: async (Client, message, args) => {
        var embedMention = new EmbedBuilder()
            .setColor("#2F3136")
            .setTitle("<:warning:869206692091531305> Veuillez mentionner le nombre de messages")
        if (!message.member.permissions.has(PermissionsBitField.Flags.ManageMessages)) return message.reply("**Vous n'avez pas la permission d'éxécuter cette commande !**");        let args2 = message.content.split(" ").slice(1);
